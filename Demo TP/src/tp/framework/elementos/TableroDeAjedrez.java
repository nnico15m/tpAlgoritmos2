...
public class TableroDeAjedrez extends JPanel implements Serializable
{
	private static final long serialVersionUID = -1246957520079405796L;
	private EstadoTableroAjedrez boardState;
	...
	public TableroDeAjedrez(EstadoTableroAjedrez boardState, GUI listener, Dimension size)
	{
		...
		{
			private EstadoTableroAjedrez boardState;
			public PaintedButton(EstadoTableroAjedrez boardState)
			{
				this.boardState = boardState;
			}
			public void paintComponent(Graphics g) {
				...
				if(x!=-1 && y!=-1)
				{
					x = processLocation(new Localizacion(x,y)).getRow();
					y = processLocation(new Localizacion(x,y)).getCol();
		...
		for(int y=0; y<squares.length; y++)
			for(int x=0; x<squares.length; x++)
			{
				...
				squares[y][x].addMouseListener(new ActualizacionDinamicaBarraDeEstado(listener));
			}
	...
	public JButton getAtLocation(Localizacion loc)
	{
	...
	public boolean hasActionListener(Localizacion loc)
	{
	...
	public void disableEverything()
	{
		for(int y=0; y<squares.length; y++)
			for(int x=0; x<squares.length; x++)
				if(hasActionListener(new Localizacion(y,x)))
	...
	public void enableSide(boolean turn)
	{
		...
		for(int y=0; y<squares.length; y++)
			for(int x=0; x<squares.length; x++)
				if(boardState.getState()[y][x]!=null && boardState.getState()[y][x].getColor()==turn && !hasActionListener(processLocation(new Localizacion(y,x))))
					getAtLocation(new Localizacion(y,x)).addActionListener(listener);
	}
	
	public void enableLocation(Localizacion loc)
	{
		if(!hasActionListener(new Localizacion(loc.getRow(),loc.getCol())))
	...
	public void highlight(Localizacion loc)
	{
	...
	{
		getAtLocation(new Localizacion(loc.getRow(),loc.getCol())).setBorder(BorderFactory.createLineBorder(Color.GREEN, 5));
	}
